...
public record SliceResponse<T>(
        @Schema(description = "조회된 데이터 목록") List<T> items,
        @Schema(description = "다음 페이지 존재 여부 (false이면 마지막 페이지)", example = "true") boolean hasNext,
        @Schema(description = "요청한 페이지 크기", example = "5") int size,
        @Schema(description = "실제 반환된 데이터 개수", example = "5") int numberOfElements,
        @Schema(
                        description =
                                """
                        다음 페이지 요청을 위한 커서 값
