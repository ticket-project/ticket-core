 ...
 * 타입이어야 한다.
 *
 * <p>공개 계약은 {@code shared.web}과 {@code shared.exception}에 둔다. 공통 실행 설정은 {@code shared.config}, 전역
 * 예외 처리는 {@code shared.exception.handler}가 소유한다. 이 모듈은 {@code sharedModules}로 모든 모듈 기동 테스트에 포함되므로,
 * bean은 이 두 내부 패키지에만 둔다. {@link CorsProperties}가 예외처럼 보이지만 아니다 — {@code @ConfigurationProperties} 값
 * 홀더는 스스로 bean을 등록하지 않고 주입받아 읽는 값 타입이며, 등록은 그 값을 쓰는 module이 한다.
 *
