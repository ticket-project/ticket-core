 ...
 * {@code shared.exception}, entity가 상속하는 JPA 기반 타입은 {@code shared.jpa}다. 공통 실행 설정은 {@code shared.config}, 전역 예외 처리는
 * {@code shared.exception.handler}가 소유한다. 이 모듈은 {@code sharedModules}로 모든 모듈 기동 테스트에 포함되므로, bean은 이 두 내부 패키지에만 둔다.
