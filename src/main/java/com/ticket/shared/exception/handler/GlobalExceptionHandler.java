 ...
 *
 * <p>업무 코드는 실패의 의미(errorCode)와 부가 정보(data)만 예외로 전달한다. HTTP 상태는 그 오류를 처리하는 쪽이 안다 - 여기서는 어느 module에도
 * 속하지 않는 공통 오류 셋 ({@link InvalidRequestException}/{@link NotFoundException}/{@link
 * InternalErrorException})의 상태를 이 handler가 고정하고, module 고유 오류의 상태는 각 module의 handler가 고정한다(예:
 * {@code BookingExceptionHandler}). 새 공통 오류가 생기지 않는 한 이 클래스는 수정하지 않는다.
 *
 * <p><b>{@link Ordered#LOWEST_PRECEDENCE}인 이유</b>: Spring은 advice를 order로 정렬한 뒤 매칭되는 메서드를 가진
 * <i>첫</i> advice에서 멈춘다. 여기 있는 {@code Exception} fallback이 먼저 잡히면 각 module의 handler가 영영 호출되지 않는다.
 * module handler는 반대로 {@link Ordered#HIGHEST_PRECEDENCE}를 쓰고 자기 module의 base 예외만 잡는다 - 그 범위는 {@code
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiResponse<Object>> handleInternalErrorException(
    ...
    private ResponseEntity<ApiResponse<Object>> toResponse(
            final HttpStatus status, final TicketException exception) {
        return toResponse(
                status, exception.getErrorCode(), exception.getMessage(), exception.getData());
    }
