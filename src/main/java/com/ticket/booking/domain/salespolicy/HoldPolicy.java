...
public class HoldPolicy {
     ...
     *
     * <p>{@code isZero}/{@code isNegative}만 보면 1초 미만과 소수 초가 통과한다 — {@code Duration.ofMillis(500)}은
     * 양수지만 {@code getSeconds()}가 0이라 hold TTL이 0으로 저장된다. 조용히 잘리는 대신 입력을 거부한다.
     */
    private static void validateHoldDuration(final Duration holdDuration) {
        ...
        if (holdDuration.getNano() != 0) {
            throw new InvalidRequestException(
                    "holdDuration은 초 단위 정수여야 합니다. 1초 미만이나 소수 초는 저장 시 잘립니다. holdDuration="
                            + holdDuration);
        }
    }
