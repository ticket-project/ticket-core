 ...
 *
 * <p>원래는 {@code SeatSelectionRedisKey}(selection 소유)에 selection·hold 키가 함께 있었다. hold를 자기 패키지로 모으며
 * key 형식은 그대로 두고 파싱 책임만 여기로 옮겼다 — 운영 중인 Redis key 문자열 형식은 바뀌지 않는다.
 */
public final class HoldRedisKey {
    ...
    public static Optional<HoldMetaKey> tryParseHoldMetaKey(final String key) {
        ...
        return matcher.matches()
                ? Optional.of(new HoldMetaKey(matcher.group(1)))
                : Optional.empty();
    }
