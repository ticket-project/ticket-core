 ...
 *
 * <p>유효 시간은 도메인이 결정하고 {@code expiresAt}으로 표현한다. 이 값을 Redis TTL로 적용하는 일은 {@code booking.hold.infrastructure}의
 * {@code RedissonHoldStore}가 한다. 같은 업무 TTL을 infra에 다시 정의하지 않는다.
 */
public record Hold(String holdKey, Long memberId, Long performanceId, List<Long> seatIds, LocalDateTime expiresAt) {
