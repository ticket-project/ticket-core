...
/**
 * 주문이 더 이상 활성 상태가 아니게 됐다는 사실을 알리는 도메인 이벤트다. 취소와 만료 모두 이 이벤트를 발행한다. booking DB 트랜잭션(주문 상태 전이·hold 이력
 * 저장)과 같은 트랜잭션에서 발행되고, Spring Modulith의 JPA event publication registry가 커밋 뒤 최소 한 번 전달을 보장한다.
 *
 * <p>entity, lazy proxy, repository, exception은 담지 않는다. 후속 처리(Redis hold 해제, WebSocket 발행)를 맡는
 * listener는 {@code orderId}로 현재 상태를 다시 조회해 처리한다.
 *
