 ...
 *
 * <p>조회는 {@code PerformanceSeat} 엔티티를 돌려주고, 응답 상태 변환과 Redis 점유 반영은 use case가 한다. 다만 <b>트랜잭션 경계는
 * 메서드마다 다르다</b> — 어느 조회가 스스로 경계를 갖는지는 각 메서드의 Javadoc에 있다.
 */
public interface PerformanceSeatRepository {
    /**
     * 판매 좌석 편성(PerformanceSeat 생성)을 저장한다. 이미 존재하는 (performanceId, seatId) 조합은 호출하는 유스케이스가 저장 전에
     * 걸러낸다 — 이 메서드는 DB unique 제약을 최후 방어선으로만 둔다.
     */
