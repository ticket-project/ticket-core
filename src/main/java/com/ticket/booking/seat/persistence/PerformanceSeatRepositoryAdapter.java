 ...
 *
 * <p><b>클래스 전체에는 {@code @Transactional}을 붙이지 않는다</b> — 조회마다 경계가 다르다. {@link #findSeatStates(Long)}만 읽기 전용 트랜잭션을 갖는 이유는
 * 계약의 Javadoc에 있다. 같은 클래스 안에서 그 메서드를 호출하면 self-invocation이라 proxy가 적용되지 않아 경계가 사라진다 — 내부에서 부르지 않는다.
 */
@Repository
