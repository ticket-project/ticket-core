 ...
 *
 * <p>좌석 수만큼 Redis 호출과 좌석 락이 생긴다. 현재 대표 규모(회차당 좌석 600석, 한 회원의 동시 선택은 회차 Hold 한도 이하)에서는 한 자릿수 좌석이라 회원별 인덱스를 새로 두지 않았다.
 * 인덱스를 두면 선택·해제·TTL 만료 세 경로에서 인덱스와 좌석 키의 정합성을 따로 맞춰야 하므로, 실제로 비용이 문제가 되는 것을 측정한 뒤에 도입한다.
 */
@Service
