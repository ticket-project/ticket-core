package com.ticket.booking.selection.endpoint.docs;
...
public interface SeatSelectionControllerDocs {
    ...
    ApiResponse<Void> selectSeat(
            @Parameter(description = "회차 ID", example = "1", required = true) @Positive
                    Long performanceId,
            @Parameter(description = "좌석 ID", example = "42", required = true) @Positive
                    Long seatId,
    ...
    @ApiResponses(
            value = {
                @io.swagger.v3.oas.annotations.responses.ApiResponse(
                        responseCode = "200",
                        description = "좌석 선택 해제 성공"),
                @io.swagger.v3.oas.annotations.responses.ApiResponse(
                        responseCode = "403",
                        description = "본인이 선택한 좌석이 아님")
            })
