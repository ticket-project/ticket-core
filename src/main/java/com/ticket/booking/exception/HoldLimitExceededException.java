 ...
 *
 * <p>{@code requestedSeatCount}·{@code maxSeatCount}는 진단 정보다. 한도 초과 여부는 계속 {@code HoldPolicy}가 판단하고 이 생성자는 다시 검증하지 않는다.
 * 한도가 없는 회차(무제한)는 애초에 이 예외를 만들지 않으므로 {@code maxSeatCount}는 여기서 non-null이다.
 */
@Getter
public final class HoldLimitExceededException extends BookingException {
    ...
    public HoldLimitExceededException(final long requestedSeatCount, final int maxSeatCount) {
