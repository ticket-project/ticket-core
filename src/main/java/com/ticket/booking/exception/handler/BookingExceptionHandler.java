 ...
 * {@code com.ticket.booking.exception.handler.BookingExceptionHandlerTest}가 15종 전부의 상태·E-code·메시지를
 * 고정한다({@code gatling-test}가 E4001·E6000·E6003 등을 하드코딩하는 외부 계약이다).
 *
 * <p>admission token 검증 오류도 여기서 잡는다. {@link AdmissionTokenException}은 {@link BookingException}의 하위 타입이 아니므로(공통 base는
 * {@code TicketException}) 별도 메서드를 둔다 — 검증 실패 사유를 admission의 어휘로 로깅하고 공개 메시지를 사유와 분리하는 정책이 booking 일반 오류와 다르기 때문이다. 세
 * 종류(required/expired/invalid) 모두 403이다.
 */
@Slf4j
...
public class BookingExceptionHandler {
    ...
    public ResponseEntity<ApiResponse<Object>> handleAdmissionTokenException(final AdmissionTokenException exception) {
        log.info(
