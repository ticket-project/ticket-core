...
/**
 * booking 오류를 응답으로 옮긴다. base 예외 하나만 잡는다 — 그 범위는 {@code
 * com.ticket.shared.exception.ExceptionHandlerScopeTest}가 강제한다.
 *
 * <p>{@link BookingException}은 상태를 모른다 — 구체 타입별 HTTP 상태는 이 handler가 안다. {@code
 * com.ticket.booking.exception.handler.BookingExceptionHandlerTest}가 15종 전부의 상태·E-code·메시지를
 * 고정한다({@code gatling-test}가 E4001·E6000·E6003 등을 하드코딩하는 외부 계약이다).
 *
 * <p>admission token 검증 오류도 여기서 잡는다. {@link AdmissionTokenException}은 {@link BookingException}의 하위
 * 타입이 아니므로(공통 base는 {@code TicketException}) 별도 메서드를 둔다 — 검증 실패 사유를 admission의 어휘로 로깅하고 공개 메시지를 사유와
 * 분리하는 정책이 booking 일반 오류와 다르기 때문이다. 세 종류(required/expired/invalid) 모두 403이다.
 */
@Slf4j
