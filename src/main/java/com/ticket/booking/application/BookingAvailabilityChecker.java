 ...
 * Redis 왕복을 포함하지 않을 만큼 짧아야 한다. {@link com.ticket.booking.application.usecase.StartBookingUseCase}가
 * 같은 클래스의 private method로 호출하면 Spring proxy가 적용되지 않아 트랜잭션이 아예 걸리지 않는다.
 *
 * <p>확인하는 것은 둘이다.
 *
 * <ul>
    ...
    private List<PerformanceSeat> requireAvailableSeats(
        ...
        final List<PerformanceSeat> performanceSeats =
                performanceSeatRepository.findAllByPerformanceIdAndSeatIdIn(
                        performanceId, requestedSeatIds.toList());
