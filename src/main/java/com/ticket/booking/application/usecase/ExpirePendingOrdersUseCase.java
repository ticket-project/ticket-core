 ...
 *
 * <p>정상 경로는 Redis TTL 만료 리스너가 즉시 처리한다. 이 유스케이스는 그 실행을 놓친 주문을 다시 만료시키는 보정 경로이며, 배치 반복과 실패 격리를 이 계층이
 * 소유한다.
 *
 * <p><b>실패한 항목을 넘어 뒤의 대상까지 처리한다.</b> 옛 구현은 매번 첫 페이지를 다시 읽어, 앞의 100건이 계속 실패하면 그 뒤의 정상 만료 대상이 영원히
 * 처리되지 않았다(한 건도 성공하지 못하면 무한 반복을 피하려고 아예 멈췄다). 지금은 id 커서로 한 번의 순회에서 실패 항목을 지나쳐 다음 페이지로 나아간다. 실패 항목은
 * 삭제하지도 처리 완료로 치지도 않으므로 PENDING으로 남아 다음 순회에서 다시 시도된다.
 */
@Slf4j
@Service
