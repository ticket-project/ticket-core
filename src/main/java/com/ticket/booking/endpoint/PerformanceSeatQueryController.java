...
import com.ticket.booking.endpoint.docs.PerformanceSeatQueryControllerDocs;
 ...
 *
 * <p>{@code PerformanceSeat}와 Redis selection/hold 상태는 booking이 소유하는 데이터라, 이 세 엔드포인트는 순수 show/공연 요약
 * 데이터만 다루는 {@code com.ticket.show.endpoint.PerformanceController}가 아니라 booking 소유인 이 {@code
 * com.ticket.booking.endpoint.PerformanceSeatQueryController}에 둔다. URL·JSON 계약은 기존과 동일하다.
 */
@RestController
...
public class PerformanceSeatQueryController implements PerformanceSeatQueryControllerDocs {
    ...
    public ApiResponse<GetPerformanceSeatMapUseCase.Output> getSeatMap(
        ...
        final GetPerformanceSeatMapUseCase.Input input =
    ...
    public ApiResponse<GetSeatAvailabilityUseCase.Output> getSeatAvailability(
            @PathVariable final Long performanceId) {
        final GetSeatAvailabilityUseCase.Input input =
                new GetSeatAvailabilityUseCase.Input(performanceId);
