...
public interface PerformanceSeatQueryControllerDocs {
    @Operation(
            ...
            description =
                    "Returns the static seat map (venue layout, seat "
                            + "coordinates, grade and price) for a performance.")
    @ApiResponses(
            value = {
    ...
    ApiResponse<GetPerformanceSeatMapUseCase.Output> getSeatMap(
            @Parameter(description = "Performance ID", example = "1", required = true) @Positive
                    Long performanceId);
