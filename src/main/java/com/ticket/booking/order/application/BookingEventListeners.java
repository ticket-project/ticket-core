 ...
 * com.ticket.shared.config.EventPublicationMaintenance}가 재시도한다.
 *
 ...
 * com.ticket.booking.application}에서 {@code com.ticket.booking.order.application}으로 옮겨지면 id가 바뀐다.
 * id가 바뀌면 EVENT_PUBLICATION에 남아 있는 미완료 publication이 어떤 listener의 것인지 매칭되지 않아 재처리도 완료 처리도 되지 않는다.
 * 그래서 {@code id}를 옛 값으로 명시해 고정한다 — 아래 문자열의 {@code booking.application}은 현재 package가 아니라 <b>호환성
 * 식별자</b>이므로 package 경로 일괄 치환 대상에서 제외한다. {@code BookingEventListenerIdContractTest}가 이 값을 고정한다.
 */
@Slf4j
@Component
