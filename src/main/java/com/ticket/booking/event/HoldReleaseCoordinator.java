 ...
 *
 * <p>해제는 {@link HoldReleaseProgressRecorder}로 한 번만 수행한다 — 이 후속 처리는 트랜잭션 없이 실행되는 listener에서 불리고 이벤트가 재전달될 수 있다. 발행 대상도 락
 * 안에서 현재 상태를 다시 확인해 고른다.
 */
@Component
...
public class HoldReleaseCoordinator {
    ...
    public void releaseAndPublish(final UUID eventId, final HoldReleaseTask task, final LocalDateTime now) {
    ...
    private void releaseAndPublishLocked(final UUID eventId, final HoldReleaseTask task, final LocalDateTime now) {
    ...
    private void releaseHoldOnce(final UUID eventId, final HoldReleaseTask task, final LocalDateTime now) {
