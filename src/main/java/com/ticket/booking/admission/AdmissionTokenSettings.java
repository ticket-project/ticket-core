 ...
 *
 * <p>만료 시간은 담지 않는다 — Core는 토큰 자체의 {@code exp} claim으로 만료를 판정하고({@code JwtAdmissionVerifier}가 Jwts
 * 파서에 clock을 넘긴다), 발급 시점의 TTL은 토큰을 만드는 {@code ticket-queue}가 정한다. 설정 키 {@code
 * security.admission.expiration-seconds}는 기존 구성 호환을 위해 {@link AdmissionTokenProperties}에 그대로 남아 있지만
 * Core의 검증 흐름에는 들어오지 않는다.
 */
