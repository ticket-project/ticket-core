 ...
 *
 * <p><b>왜 별도 class인가.</b> 같은 조회가 다섯 곳(예매 시작, 좌석 선택, 좌석 상태 조회, 잔여석 조회, 예매 방식 조회)에서 필요하고, 다섯 곳 모두 "없으면 404" 라는 같은 결론을 낸다.
 * 같은 결론을 다섯 번 적으면 문구가 어긋나는 순간 클라이언트가 보는 메시지가 화면마다 달라진다 -- 그런데 그 어긋남은 컴파일러도 테스트도 잡아 주지 않는다.
 *
 * <p><b>왜 Repository에 두지 않았나.</b> {@link PerformanceSalesPolicyRepository}는 "조회 결과가 없다는 사실만 알려 주고, 그것을 어떤 오류로 볼지는 호출하는
