...
/**
 * 회차 하나의 예매 접수 기간·Hold 제한·대기열 진입 정책을 소유하는 Booking BC aggregate다(ADR 0006 "Performance의 책임 혼재" A2).
 * {@code performanceId}는 show가 소유한 {@code Performance}에 대한 scalar 식별자일 뿐 cross-module JPA 연관관계나 DB
 * FK가 아니다.
 *
 * <p>정책 조회 실패("이 회차는 Booking 판매 정책이 구성되지 않음")는 이 aggregate가 아니라 그것을 조회하는 use case가 {@link
 * com.ticket.shared.exception.NotFoundException}으로 판단한다 — 이 도메인은 존재 여부를 스스로 판단하지 않는다({@link
 * com.ticket.shared.exception.NotFoundException} 계약 참고).
 */
@Getter
@Entity
...
public class PerformanceSalesPolicy extends AuditedEntity {
    ...
    @Embedded private OrderAcceptanceWindow orderAcceptanceWindow;
    @Embedded private HoldPolicy holdPolicy;
    @Embedded private BookingEntryPolicy bookingEntryPolicy;
    ...
    /**
     * 예매 접수 기간 안인지 확인한다. 옛 {@code BookingPolicyGuard.ensureBookingOpen}과 같은 오류(E3002/E3001)를 그대로
     * 던진다.
     */
    ...
    /**
     * 한도가 없는 회차는 좌석 수를 제한하지 않는다. 옛 {@code BookingPolicyGuard.ensureWithinHoldLimit}와 같은 오류(E6001)를
     * 그대로 던진다.
     */
