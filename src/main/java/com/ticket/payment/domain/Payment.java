 ...
 *
 * <p>이번 entity-only 단계는 PG client, 승인/실패/취소 API, callback/webhook을 구현하지 않는다. 아래 상태 전이 메서드는 그 자체가 PG 연동이 아니라, Payment
 * aggregate가 자신의 상태 불변식을 스스로 지키게 하는 도메인 규칙이다(추후 후속 작업이 이 메서드를 호출한다).
 */
@Getter
@Entity
@Table(
