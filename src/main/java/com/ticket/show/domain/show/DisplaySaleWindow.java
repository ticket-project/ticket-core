...
/**
 * Show가 화면에 보여주는 판매 기간이다. 실제 주문 접수 가능 여부는 이 값이 아니라 booking의 {@code PerformanceSalesPolicy}가 회차 단위로 판단한다 — 이 값은
 * 목록·검색·상세에 쓰이는 표시 전용 데이터이고, booking의 판단과 정합성 검증 없이 독립적으로 존재한다(ADR 0007). 그래서 {@code /shows/{id}}가 ON_SALE이라 응답해도 주문
...
public class DisplaySaleWindow {
    ...
    public DisplaySaleWindow(final @Nullable LocalDateTime startsAt, final @Nullable LocalDateTime endsAt) {
        this.startsAt = startsAt;
        this.endsAt = endsAt;
    }

    /**
     * null 창(시작·종료 중 하나라도 없음)은 "표시 정책이 아직 구성되지 않았다"는 뜻이라 {@code CLOSED}로 본다 — {@code Show.getBookingStatus}의 예전 규칙과 같다.
     */
