...
/**
 * 내 찜 목록을 조회한다. favorite가 찜 항목(likeId·showId·likedAt)을 커서 페이지로 주면,
 * 그 showId 집합으로 show 자기 데이터를 다시 조회해 표시값(제목·이미지·공연장 이름)을 조립한다 —
 * favorite는 show 표시값을 모른다.
 *
 * <p>표시값을 찾지 못한 showId(삭제된 공연)는 목록에서 조용히 건너뛴다. {@code hasNext}/
 * {@code nextPosition}은 favorite가 준 페이지 값을 그대로 쓰므로, 건너뛴 항목이 있으면 이번
...
public class GetMyShowLikesUseCase {
    ...
    public Output execute(final Input input) {
        ...
        final CursorPage<ShowLikeEntry, Long> page =
                showLikeQuery.findLikedShows(input.memberId(), input.cursorLikeId(), input.size());
        ...
        final List<ShowLikeSummaryView> items = page.items().stream()
                .map(entry -> toSummaryView(entry, summaries.get(entry.showId())))
    ...
    private ShowLikeSummaryView toSummaryView(final ShowLikeEntry entry, final ShowSummaryRow summary) {
        if (summary == null) {
            return null;
        }
