...
public class GetMyShowLikesUseCase {
    ...
    public Output execute(final Input input) {
        ...
        final Map<Long, ShowSummaryRow> summaries = showSummaryBatchQueryPort.findSummaries(showIds);
        final VenueDisplays venues = VenueDisplays.load(venueLookup, summaries.values().stream().map(ShowSummaryRow::venueId).toList());
    ...
    private ShowLikeSummaryView toSummaryView(final LikeEntry entry, final ShowSummaryRow summary, final VenueDisplays venues) {
        if (summary == null) {
            return null;
        }
