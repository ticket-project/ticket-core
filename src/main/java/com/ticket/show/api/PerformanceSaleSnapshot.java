 ...
 *
 * <p>{@code seatInfoBySeatId}는 호출자가 넘긴 seat ID 중 이 회차의 Venue에 실제로 속한 좌석만 채워진다. {@code gradeInfoByPerformanceGradeId}는 이
 * 회차에 배정된 모든 PerformanceGrade를 담는다. show JPA entity를 노출하지 않는다.
 */
public record PerformanceSaleSnapshot(
    ...
    public record GradeInfo(
            long performanceGradeId, String gradeCode, String gradeName, int sortOrder, BigDecimal price) {}
}
