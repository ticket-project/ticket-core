 ...
 *
 * <p>{@code seatLayoutBySeatId}는 이 회차의 Venue에 속한 모든 물리 Seat를 담는다 — 실제로 이 회차에 판매
...
public record PerformanceVenueLayout(
    ...
    /**
     * 회차 하나에 배정된 PerformanceGrade 하나의 표시값이다. 가격은 담지 않는다 — seat-map 응답의 좌석별 가격은 판매 시점에 확정돼 불변인
     * booking {@code PerformanceSeat.unitPrice}가 원본이다.
     */
    public record GradeLayout(
            long performanceGradeId, String gradeCode, String gradeName, int sortOrder) {}
}
