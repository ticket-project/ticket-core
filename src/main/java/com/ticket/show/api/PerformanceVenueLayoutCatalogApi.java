...
/**
 * booking이 정적 seat-map을 조합하는 데 필요한 show 쪽 조회를 모은 공개 계약이다. 회차 단위 Venue 배치·물리 Seat 좌표· PerformanceGrade 표시값과, 공연(Show) 단위
 * 진입점이 쓰는 대표 회차 식별자가 함께 있다.
 *
 * <p><b>대표 회차 조회를 따로 떼지 않는다.</b> 같은 조회({@code PerformanceRepository})를 같은 소비자 ({@code booking.seat}의 seat-map use
