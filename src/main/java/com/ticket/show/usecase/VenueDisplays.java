...
/**
 * show 목록·상세 조회가 venue 표시값(이름·지역)을 배치로 채울 때 쓰는 작은 값 객체다. venueId가 null이거나(venue 없는 show) venue module에 실제로 없는 dangling
 * id여도 예외 없이 null을 돌려준다 — 둘 다 "venue 없는 show"와 같은 값으로 통일한다(과거 {@code leftJoin} 결과와 동일).
 */
public record VenueDisplays(Map<Long, VenueSnapshot> byId) {
    ...
    public @Nullable VenueSnapshot get(final @Nullable Long venueId) {
