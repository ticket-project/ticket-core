 ...
 *   <li>{@code evaluatedAt} — 그 마감 여부를 판정한 시각. 첫 페이지에서 고정해 이후 페이지가 그대로 쓴다. 페이지마다 현재 시각으로 다시 판정하면, 페이지를 넘기는 사이 마감된 공연이 뒷
 *       그룹으로 옮겨가 건너뛰거나 중복된다.
 * </ul>
 *
...
public record ShowCursor(
    ...
    public ShowCursor(final ShowSort sort, final String dir, final String lastValue, final Long lastId) {
