 ...
 * 아니다.
 *
 * <p>동적 조건·커서 페이징·집계가 필요한 조회는 {@code ShowQuerydslRepository}가 Querydsl로 갖는다(공연 목록·검색·오픈 예정·가격 집계).
 * 고정 조회는 계약({@code *Repository})과 그 adapter로 내려가 있다. 밖으로 나가는 것은 entity와 DB 집계 결과뿐이고, 응답 항목은 그것을 쓰는
 * use case가 소유한다({@code show.usecase}).
 */
@NullMarked
