 ...
 * Querydsl 조립은 {@code ShowQueryRepository} 안에만 있고, 고정 projection 조회는 {@code
 * SpringData*JpaRepository}의 {@code @Query}가 갖는다({@code docs/readability-guidelines.md} §10). 밖으로는
 * entity와 DB 집계 결과만 나간다 — 응답 항목은 그것을 쓰는 use case가 소유한다({@code show.usecase}).
 */
@NullMarked
