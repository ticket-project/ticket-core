...
public interface LikeQueryApi {
    ...
    /** 특정 회원이 찜한 대상 목록을 최신순(likeId 내림차순)으로 한 페이지 조회한다. {@code cursorLikeId}는 이전 페이지 마지막 항목의 likeId다(첫 페이지는 null). */
    CursorPage<LikeSnapshot, Long> findLiked(LikeType likeType, long memberId, @Nullable Long cursorLikeId, int size);
}
