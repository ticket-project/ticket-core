 ...
 *
 * <p>대상 종류는 {@link com.ticket.like.api.LikeType}으로 값화돼 있다. 지금은 {@code SHOW} 하나뿐이지만 공연장·출연자 찜이 생겨도 이 module과 테이블(LIKES)을
 * 늘리지 않고 값만 추가한다. 다만 대상별 표시값 조립(예: "내 찜 목록"에 보여줄 공연 제목·이미지)은 여전히 그 대상을 아는 module의 책임이다 — 이 module은 줄여주지 않는다.
 *
 ...
 *
 * <p>공개 계약: - LikeQueryApi (찜 여부·개수·내 찜 목록 조회) - LikeType (찜 대상 종류). 찜하기·찜 해제는 이 module의 HTTP endpoint와 use case로만 쓰고
 * 밖에 노출하지 않는다.
 *
