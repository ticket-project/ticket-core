...
public interface VenueSeatLookupApi {
    /** venue에 속한 좌석 중 요청한 seatId만 반환한다. 빈 {@code seatIds}는 빈 목록을 반환한다. */
    List<VenueSeatSnapshot> findSeats(long venueId, Set<Long> seatIds);

    /** venue에 속한 모든 물리 좌석의 배치 좌표를 반환한다. */
    List<VenueSeatSnapshot> findAllSeatLayouts(long venueId);
}
