 ...
 * 수도 있다.
 *
 * <p>소셜 계정({@code MemberSocialAccount})은 회원 aggregate의 자식이라 자기 Repository를 갖지 않는다. 자식을 이미 손에 쥔 조회는
 * {@code Member}의 컬렉션 접근 메서드로 하고, 아래 {@link #findActiveBySocialAccount}처럼 <b>자식 속성으로 Root를 찾는
 * 조회만</b> 이 Repository가 맡는다 — Repository가 반환하는 것은 언제나 aggregate root다.
 */
