...
/**
 * 소셜 계정은 회원 aggregate의 자식이므로 별도 Repository를 거치지 않는다. 소셜 ID로 회원을 찾는 조회만 {@link MemberRepository}가 맡고, 연결 여부 확인과 연결 추가는
 * {@code Member}가 자기 컬렉션으로 처리한다. provider가 검증한 이메일만 기존 계정 자동 연결과 회원 이메일에 사용한다.
 */
@Service
...
public class SocialAccountProvisioningService {
     ...
     *   <li>없으면 이메일로 기존 회원을 찾는다. <b>provider가 검증한 이메일만</b> 쓰고, 검증되지 않았으면 provider ID 기반 대체 주소로 격리해 남의 계정에 붙지 않게 한다.
    ...
    public Member getOrCreateMember(final SocialIdentity identity) {
        final Optional<Member> linkedMember =
                memberRepository.findActiveBySocialAccount(identity.provider(), identity.providerId());
        ...
        if (memberWithSameEmail.isEmpty()) {
            final Member member = Member.createSocialMember(Email.create(email), resolveName(identity), Role.MEMBER);
            member.addSocialAccount(identity.provider(), identity.providerId());
            return memberRepository.save(member);
        }

        final Member existingMember = memberWithSameEmail.get();
        final Optional<MemberSocialAccount> linkedAccount = existingMember.findActiveSocialAccount(identity.provider());
    ...
    private String resolveEmail(final SocialIdentity identity) {
        ...
        return identity.provider().name().toLowerCase() + "_" + identity.providerId() + "@social.ticket";
    }
