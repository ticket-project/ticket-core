 ...
 *
 * <p>각 연산이 <b>자기 트랜잭션을 직접 소유한다.</b> 다른 module은 이 계약을 인터페이스로 주입받아 밖에서 호출하므로 Spring proxy를 그대로 통과한다 — 이 클래스 안에서 자기 public
 * method를 다시 부르면 그 순간 트랜잭션이 사라진다(self-invocation).
 *
 * <p>비밀번호 해시는 이 경계 밖으로 나가지 않는다. 해싱과 일치 확인을 member가 직접 하므로 {@code member -> security} 의존이 생기지 않는다. 반대로 <b>외부 provider 연결
 * 해제는 여기서 하지 않는다</b> — 탈퇴는 DB 처리만 하고 연결 목록을 돌려주며, 실제 unlink는 security가 커밋 뒤에 수행한다. 외부 호출을 DB 트랜잭션 안으로 끌어들이지 않기 위해서다.
 *
 * <p>소셜 계정 해석만 {@link OAuth2MemberProvisioningService}가 따로 소유한다. 검증된 이메일 판정, 기존 계정 연결, provider 충돌 판단이 독립적으로 복잡한 정책이라
 * 흐름을 가리지 않고 이름으로 드러나는 편이 낫다.
 */
@Service
...
public class MemberAccountService implements MemberAccountApi {
     ...
     * {@inheritDoc}
     *
     * <p><b>없는 계정과 비밀번호 불일치를 구분하지 않는다.</b> 둘 다 같은 {@link UnauthenticatedException}(401, E1000)을 던지고, 없는 계정에도 더미 해싱을 수행해
     * 응답 시간까지 맞춘다. 저장된 비밀번호가 없는 소셜 전용 회원도 같은 실패로 다룬다.
     */
    @Override
     ...
     * {@inheritDoc}
     *
     * <p>소셜 계정은 회원 aggregate의 자식이라 컬렉션으로 접근한다. row를 지우지 않고 {@code deletedAt}만 채우는 <b>soft delete</b>이며, 외부 provider 연결
     * 해제에 쓸 식별자는 <b>탈퇴 처리로 값이 바뀌기 전에 먼저 모아 둔다</b> — 순서를 뒤집으면 익명화된 식별자로 unlink를 보내게 된다. aggregate 내부 상태 변경은
     * {@link Member#withdraw}가 한 번에 수행한다.
     */
    @Override
    @Transactional
