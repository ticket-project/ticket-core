...
public class CustomOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
    /**
     * 회원 식별자를 담는 attribute 이름이다. DefaultOAuth2User의 nameAttributeKey로 지정해 Authentication.getName()이 회원 식별자를 돌려주게 한다.
     */
    ...
    public OAuth2User loadUser(final OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        final SocialIdentity userInfo = OAuth2UserInfoMapper.map(registrationId, oauth2User.getAttributes());
        ...
        return new DefaultOAuth2User(
                List.of(new SimpleGrantedAuthority("ROLE_" + member.role())), attributes, MEMBER_ID_ATTRIBUTE);
    }
}
