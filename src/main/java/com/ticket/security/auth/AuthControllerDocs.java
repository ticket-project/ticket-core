...
public interface AuthControllerDocs {
    ...
    @Operation(
            summary = "로그인",
            description =
                    "이메일과 비밀번호로 로그인하고 Access Token을 응답으로 반환합니다. Refresh Token은 HttpOnly "
                            + "쿠키로 설정됩니다.")
    @ApiResponses(
            value = {
                @io.swagger.v3.oas.annotations.responses.ApiResponse(
    ...
    ApiResponse<LoginUseCase.Output> login(
            @Valid LoginRequest request, @Parameter(hidden = true) HttpServletResponse response);

    @Operation(
            summary = "토큰 재발급",
            description =
                    "HttpOnly 쿠키의 Refresh Token으로 Access Token을 재발급하고, 새로운 Refresh "
                            + "Token으로 쿠키를 교체합니다.")
    @ApiResponses(
            value = {
                @io.swagger.v3.oas.annotations.responses.ApiResponse(
    ...
    @Operation(
            ...
            description =
                    "소셜 로그인 성공 후 발급된 1회용 인증 코드를 Access Token으로 교환합니다. Refresh "
                            + "Token은 HttpOnly 쿠키로 설정됩니다.")
    @ApiResponses(
            value = {
                @io.swagger.v3.oas.annotations.responses.ApiResponse(
                        responseCode = "200",
