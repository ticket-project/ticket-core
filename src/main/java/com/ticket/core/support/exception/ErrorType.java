...
/**
 * 전역 오류 카탈로그다. 하나의 오류가 외부에 보이는 완성된 계약(HTTP 상태, 코드, 공개 메시지)을
 * 한 번에 제공하므로 호출부는 {@code throw new CoreException(ErrorType.SEAT_ALREADY_HOLD)}처럼
 * 완성된 오류를 고르기만 한다.
 *
 * <p>기능 오류를 추가할 때 {@link ApiControllerAdvice}는 수정하지 않는다.
 */
@Getter
public enum ErrorType {

    DEFAULT_ERROR(HttpStatus.INTERNAL_SERVER_ERROR, ErrorCode.E500, "일시적인 오류가 발생했습니다."),
    ...
    API_NOT_FOUND(HttpStatus.NOT_FOUND, ErrorCode.E404, "요청한 API를 찾을 수 없습니다."),
    ...
    NOT_SUPPORT_SHOW_SORT(HttpStatus.BAD_REQUEST, ErrorCode.E7002, "지원하지 않는 정렬 조건입니다."),
