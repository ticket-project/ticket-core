...
-- (docs/operations.md 참고). catalog의 다른 migration 검증 baseline(예: CatalogCommonCodeSchemaTest,
-- OracleMigrationCompatibilityTest)은 SHOW_GRADES를 아직 모르는 legacy 상태를 재현하므로, 이
-- migration은 catalog V1(동적 FK 제거)과 같은 패턴으로 SHOW_GRADES 존재 여부를 먼저 확인하고 없으면
-- no-op이다.
--
...
-- ShowGradePerformanceGradeBackfillMigrationTest가 검증한다.
