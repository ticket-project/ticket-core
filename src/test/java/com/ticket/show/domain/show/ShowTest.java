...
/**
 * 판매 표시 상태 판정 자체의 경계값·null 케이스는 {@link DisplaySaleWindowTest}가 고정한다. 여기서는 {@link
 * Show#saleDisplayStatusAt}이 {@code DisplaySaleWindow}에 위임하는지와 생성자가 표시 필드를 올바르게 채우는지만 확인한다.
 */
...
class ShowTest {
    ...
    private Show createShow(
            final LocalDateTime displaySaleStartsAt, final LocalDateTime displaySaleEndsAt) {
        return new Show(
                "공연",
                "부제",
                "소개",
