...
class GetGenresByCategoryUseCaseTest {
    ...
    @InjectMocks private GetGenresByCategoryUseCase useCase;
    ...
    void 카테고리코드가_비어있으면_전체_장르를_조회한다() {
        ...
        // when
        GetGenresByCategoryUseCase.Output output =
                useCase.execute(new GetGenresByCategoryUseCase.Input(" "));
        // then
    ...
    void 카테고리코드가_null이면_전체_장르를_조회한다() {
        ...
        // when
        GetGenresByCategoryUseCase.Output output =
                useCase.execute(new GetGenresByCategoryUseCase.Input(null));
        // then
        assertThat(output.genres()).extracting("code").containsExactly("KPOP");
        verify(genreRepository).findAllOrderByCategoryAndName();
    }
    ...
    void 카테고리코드가_있으면_해당_장르만_조회한다() {
        // given
        Genre genre = createGenre(1L, "KPOP", "케이팝");
        when(genreRepository.findAllByCategoryCodeOrderByName("CONCERT"))
                .thenReturn(List.of(genre));
        // when
        GetGenresByCategoryUseCase.Output output =
                useCase.execute(new GetGenresByCategoryUseCase.Input("CONCERT"));
        // then
