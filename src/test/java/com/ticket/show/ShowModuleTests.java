 ...
 *
 * <p>STANDALONE bootstrap mode는 {@code com.ticket.show} package tree만 component-scan한다. {@code shared}는
 * {@code @Modulith(sharedModules = "shared")} 덕에 이 테스트에도 포함되지만 이제 호출 대상 계약만 갖고 bean을 등록하지 않으므로(전역 기술 설정은
 * {@code com.ticket.shared.infrastructure}가 소유한다), 스캔 범위 밖에서 오는 {@code JPAQueryFactory}와 {@code Clock}은
 * {@code @MockitoBean}으로 대체한다 — 이 테스트는 show bean들이 module 경계 안에서 서로 정상 배선되는지만 확인하는 wiring smoke test이지 실제 DB 접근이나 시간
 * 계산을 검증하지 않는다. 그 검증은 각 Querydsl repository의 통합 테스트와 use case 테스트가 담당한다.
 *
 * <p>show는 회원 존재 확인을 위해 member의 {@link MemberLookupApi}을 참조하고, 찜의 데이터·불변식을 소유하는 like(옛 favorite)의 {@link LikeQueryApi}를
 * 참조한다(공연 상세의 찜 개수, 내 찜 목록 조회). 찜하기·찜 해제는 ADR 0009로 like가 소유하므로 show는 쓰기 계약을 참조하지 않는다. 물리 공연장·좌석을 소유하는 venue의
