 ...
 * {@code com.ticket.shared.config}가 소유한다), 스캔 범위 밖에서 오는 {@code JPAQueryFactory}와 {@code Clock}은 {@code @MockitoBean}으로
 * 대체한다 — 이 테스트는 show bean들이 module 경계 안에서 서로 정상 배선되는지만 확인하는 wiring smoke test이지 실제 DB 접근이나 시간 계산을 검증하지 않는다. 그 검증은 각
 * Querydsl repository의 통합 테스트와 use case 테스트가 담당한다.
 *
