...
class CountSearchShowsUseCaseTest {
    ...
    void 검색_개수를_응답으로_감싼다() {
        ...
        when(showListQueryPort.countSearchShows(request)).thenReturn(42L);
        ...
        verify(showListQueryPort).countSearchShows(request);
    }

    @Test
    void 검색결과가_없으면_0건을_반환한다() {
        ShowSearchCriteria request = new ShowSearchCriteria("없는공연", null, null, null, null, null, null);
        when(showListQueryPort.countSearchShows(request)).thenReturn(0L);
