 ...
 *
 * <p><b>왜 필요한가</b>: Spring은 {@code @ControllerAdvice} bean들을 order로 정렬한 뒤 매칭되는 메서드를 가진 <i>첫</i> advice에서 멈춘다 — 그 안에서 더
 * 구체적인 handler를 다른 advice가 갖고 있어도 소용없다. module handler는 모두 {@code HIGHEST_PRECEDENCE}이므로, 그중 하나가
 * {@code RuntimeException}처럼 넓은 타입을 잡으면 <b>다른 module의 오류까지 삼켜</b> 엉뚱한 상태와 E-code로 응답한다. 컴파일과 기존 테스트는 전부 통과하고 운영에서만 드러나는
 * 종류의 사고다.
 *
 * <p>{@code com.ticket.shared.exception.handler}의 전역 handler는 대상이 아니다 — 그쪽은 프레임워크 예외와 {@code Exception} fallback을 잡는 것이
...
class ExceptionHandlerScopeTest {
    ...
    private static final String GLOBAL_HANDLER_PACKAGE = "com.ticket.shared.exception.handler";
            ...
            for (final JavaMethod method : advice.getMethods()) {
                ...
                for (final JavaClass parameter : method.getRawParameterTypes()) {
                    if (!parameter.getPackageName().startsWith(modulePackage)) {
                        violations.add("%s#%s가 %s를 잡는다"
                                .formatted(advice.getSimpleName(), method.getName(), parameter.getName()));
                    }
                }
            }
        }

        assertThat(checkedHandlers).as("module handler가 하나도 안 잡히면 이 테스트가 무의미하다").isPositive();
