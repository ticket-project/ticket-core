 ...
 * 분석 대상에서 뺀다(아직 8개 Application Module로 옮기지 못한 코드라서). 즉 Spring Modulith의
 * {@code ApplicationModules.verify()}는 이 package 안의 계층 방향을 전혀 검사하지 않으므로, 이
 * ArchUnit rule을 지워도 그 자리를 대신 채워줄 검증이 없다. 코드가 8개 module로 완전히 옮겨져
...
class CoreLayerArchitectureTest {
     ...
     * 공통 오류는 {@code com.ticket.error} module이, 업무 오류는 각 module의 {@code internal.exception}이 소유한다(ADR 0002는 이 결정으로
