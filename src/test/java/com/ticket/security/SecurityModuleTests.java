 ...
 * 내부가 아니라 이 계약에만 기대는지가 이 테스트로 드러난다.
 *
 * <p>access token 읽기 계약은 더 이상 mock으로 대체하지 않는다. JWT 구현이 이 module 안({@code security.jwt})으로 들어와, 계약을
 * mock으로 바꾸면 같은 module의 발급기가 쓰는 실제 codec까지 사라진다.
 */
