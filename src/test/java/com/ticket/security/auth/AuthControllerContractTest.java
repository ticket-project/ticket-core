...
class AuthControllerContractTest {
    ...
    private final RefreshAuthTokenUseCase refreshAuthTokenUseCase =
            Mockito.mock(RefreshAuthTokenUseCase.class);
    ...
    void 리프레시_API는_성공_응답과_refresh_cookie를_내린다() throws Exception {
        when(refreshAuthTokenUseCase.execute(any(RefreshAuthTokenUseCase.Input.class)))
                .thenReturn(
                        new RefreshAuthTokenUseCase.Result(
                                new RefreshAuthTokenUseCase.Output(
                                        "new-access-token", "Bearer", 1800L, 1L),
                                "new-refresh-token",
                                1209600L));
    ...
    void OAuth2_token_API는_성공_응답과_refresh_cookie를_내린다() throws Exception {
        when(exchangeOAuth2TokenUseCase.execute(any(ExchangeOAuth2TokenUseCase.Input.class)))
                .thenReturn(
                        new ExchangeOAuth2TokenUseCase.Result(
                                new ExchangeOAuth2TokenUseCase.Output(
                                        "oauth-access-token", "Bearer", 1800L, 7L),
                                "oauth-refresh-token",
                                1209600L));
