...
class AuthControllerContractTest {
    ...
    void 리프레시_API는_성공_응답과_refresh_cookie를_내린다() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/auth/refresh").cookie(new MockCookie("refresh_token", "refresh-token")))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.result").value("SUCCESS"))
                ...
                .andExpect(jsonPath("$.data.memberId").value(1))
                .andExpect(header().string(
                                "Set-Cookie", org.hamcrest.Matchers.containsString("refresh_token=new-refresh-token")))
                .andExpect(jsonPath("$.error").isEmpty());
    }
    ...
    void OAuth2_token_API는_성공_응답과_refresh_cookie를_내린다() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/auth/oauth2/token")
                ...
                .andExpect(header().string(
                                "Set-Cookie",
                                org.hamcrest.Matchers.containsString("refresh_token=oauth-refresh-token")))
                .andExpect(jsonPath("$.error").isEmpty());
    }
    ...
    void 로그아웃_API는_성공_응답과_쿠키_삭제를_내린다() throws Exception {
        ...
        when(logoutUseCase.execute(any(LogoutUseCase.Input.class))).thenReturn(new LogoutUseCase.Output());
