...
/**
 * 이 API는 인증 없이 조회된다({@code ApiSecurityConfig}의 permitAll) — 이 계약 테스트는 controller 응답 형태만 고정하고 인증 필터는
 * 검증 대상이 아니다.
 */
class PerformanceBookingModeControllerContractTest {
    ...
    void 접수중이고_대기열이_필요없으면_DIRECT를_반환한다() throws Exception {
        when(getPerformanceBookingModeUseCase.execute(
                        new GetPerformanceBookingModeUseCase.Input(10L)))
                .thenReturn(
                        new GetPerformanceBookingModeUseCase.Output(
                                10L,
