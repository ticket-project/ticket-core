 ...
 *
 * <p><b>계층 방향 규칙은 여기 없다.</b> "domain은 조립·저장·HTTP를 모른다"류는 booking만의 사정이 아니라 역할 package를 가진 모든
 * module에 같은 뜻이라, {@code com.ticket.ArchitectureRulesTest}가 역할 이름 패턴({@code ..domain..}/{@code
 * ..usecase..}/{@code ..persistence..} …) 하나로 전부 덮는다.
 *
 * <p>여기 남은 규칙이 전역으로 올라가지 않는 이유: 대상이 {@code booking.concurrency} 한 package이고, 다른 module에는 대응하는
 * package가 없다. 락 계약은 전역 규칙이 아는 역할 이름 어디에도 속하지 않아 그 패턴 밖이다.
 */
@AnalyzeClasses(
...
class BookingLayerDependencyTest {
     ...
     *
     * <p>하위 package를 포함하는 {@code ..} 없이 이 package <b>하나만</b> 가리킨다. Redisson 구현이 바로 아래 {@code
     * booking.concurrency.redis}에 있어, {@code ..}를 붙이면 구현이 스스로를 검사 대상에 넣어 규칙이 항상 실패한다.
     */
