 ...
 *
 * <p>각 단계에서 응답만 보지 않고 좌석 상태를 다시 조회한다. 좌석 상태는 DB 상태와 Redis 점유를 합쳐 계산하므로(GetSeatStatusUseCase), 여기서
 * 어긋나면 층 사이 연결이 깨진 것이다. 단위 테스트는 각 층을 따로 보기 때문에 이 어긋남을 통과시킨다.
 */
...
class BookingHappyPathE2ETest extends BookingE2ETestSupport {
    ...
    void 좌석을_고르고_주문했다가_취소하면_좌석이_돌아온다() {
        ...
        final ResponseEntity<JsonNode> select =
                post(
                        "/api/v1/performances/" + PERFORMANCE_ID + "/seats/" + seatId + "/select",
                        token);
        ...
        final ResponseEntity<JsonNode> create =
