 ...
 *
 * <p>각 단계에서 응답만 보지 않고 좌석 상태를 다시 조회한다. 좌석 상태는 DB 상태와 Redis 점유를 합쳐 계산하므로(GetSeatStatusUseCase), 여기서 어긋나면 층 사이 연결이 깨진 것이다.
 * 단위 테스트는 각 층을 따로 보기 때문에 이 어긋남을 통과시킨다.
 */
...
class BookingHappyPathE2ETest extends BookingE2ETestSupport {
    ...
    void 기존_프론트가_공연_가격과_좌석_배치도를_조회할_수_있다() {
        final ResponseEntity<JsonNode> show = restTemplate.getForEntity("/api/v1/shows/" + SHOW_ID, JsonNode.class);
        ...
        final JsonNode firstSeat =
                requireData(seatMap.getBody(), "공연별 좌석 배치도").get("seats").get(0);
    ...
    void 좌석을_고르고_주문했다가_취소하면_좌석이_돌아온다() {
        ...
        assertThat(select.getStatusCode()).as("좌석 선택 응답: %s", select.getBody()).isEqualTo(HttpStatus.OK);
        ...
        assertThat(seatStatus(token, SEAT_IDS.get(1))).as("고르지 않은 좌석까지 점유로 보인다").isEqualTo(SEAT_AVAILABLE);
        ...
        assertThat(create.getStatusCode()).as("주문 생성 응답: %s", create.getBody()).isEqualTo(HttpStatus.CREATED);
        assertThat(create.getHeaders().getFirst("X-Order-Key"))
                .as("주문 식별자를 헤더로 돌려주지 않는다")
                .isNotBlank();

        final String orderKey =
                requireData(create.getBody(), "주문 생성").get("orderKey").asText();
        ...
        pollUntil("주문한 좌석이 hold로 점유 유지", ASYNC_TIMEOUT, () -> SEAT_OCCUPIED.equals(seatStatus(token, seatId)));
        assertThat(seatStatus(token, SEAT_IDS.get(1))).as("주문하지 않은 좌석까지 점유됐다").isEqualTo(SEAT_AVAILABLE);
        // 6. 취소.
        final ResponseEntity<JsonNode> cancel = delete("/api/v1/orders/" + orderKey, token);
        assertThat(cancel.getStatusCode()).as("주문 취소 응답: %s", cancel.getBody()).isEqualTo(HttpStatus.OK);
