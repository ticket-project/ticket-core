...
/**
 * {@code spring.modulith.runtime.flyway-enabled=true}가 실제로 등록하는 {@link
 * SpringModulithFlywayMigrationStrategy}를 테스트에서 재사용하기 위한 헬퍼다. 운영 코드와 같은 class를 직접 호출해, 테스트가 실제
 * module-aware Flyway split mechanism과 어긋나지 않게 한다.
 */
public final class ModulithFlywayTestSupport {
    ...
    /**
     * {@code __root}와 주어진 module들의 migration을 각각 독립된 {@code flyway_schema_history[_module]} table로
     * 실행한다.
     */
    public static void migrate(final String url, final List<String> moduleIdentifiers) {
        ...
        final ApplicationModuleIdentifiers identifiers =
                ApplicationModuleIdentifiers.of(
