 ...
 * Modulith 구조 검증이다.
 *
 * <p>이 클래스는 두 가지를 확인한다: (1) actual dependency == {@link #APPROVED_DEPENDENCY_DAG} (architecture drift detection — 의존이 늘
 * 때도 줄 때도 실패한다. 줄었으면 이 스냅샷도 함께 줄여 "지금 무엇이 실제 edge인가"를 코드 한 곳에서 읽을 수 있게 유지한다), (2) 모든 Application Module이 CLOSED(하위
 * package 캡슐화) 상태인지. {@code verifiesModuleStructure()}는 이와 별개로 Modulith {@code verify()}의 {@code actual ⊆
 * allowedDependencies} 검사 (architectural safety — 각 module의 {@code @ApplicationModule(allowedDependencies = ...)}가 정한
 * 상한 위반만 잡는다)를 돌린다. 두 검사는 서로 다른 것을 보장하므로 하나로 합치지 않는다. 모듈 구성과 역사(BC 재편, module 신설·흡수·제거 이력)는
 * {@code docs/architecture.md}와 {@code docs/adr/0003-spring-modulith-application-module-boundaries.md} /
 * {@code docs/adr/0006-bounded-context-module-boundaries.md} /
 * {@code docs/adr/0005-performance-grade-price-ownership-and-payment-ticketing-modules.md}가 원본이다.
 *
 * <p>{@code shared}의 {@code package-info.java}가 class가 없던 시점부터 {@code @ApplicationModule}을 명시 선언해 온 이유는 {@code shared}
 * package-info의 javadoc 참고(javac가 애노테이션 없는 {@code package-info.java}는 class 파일을 만들지 않아 Modulith가 존재 자체를 못 본다).
 *
...
class ModularityTests {
     ...
     * 선언한 상한이 아니라 관측되는 edge를 여기 고정해 결합이 조용히 늘어나는 것을 잡는다. shared의 공개 하위 패키지에 대한 참조는 모두 shared edge 하나로 기록한다. 각 edge의 근거는
     * 참조하는 module의 package-info와 ADR 0006·ADR 0008(like 개명·일반화)을 원본으로 본다.
     */
