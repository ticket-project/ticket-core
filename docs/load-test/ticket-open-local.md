...
- Ticket Server 공연 상세 응답의 DIRECT/QUEUE 판단
...
- `-DadmissionTokenMode=synthetic`: Gatling이 임의의 subject와 `performanceId`로 admission token을 생성한다. Ticket Server는 subject를 access token과 비교하지 않는다.
...
Queue Server에서 admission token을 직접 확보해 쓰려면 `admissionTokenMode=tokens`를 사용한다. admission token은 요청 대상과 같은 `performanceId`여야 하며, subject는 access token의 회원과 일치할 필요가 없다.
...
active_ttl_seconds = 사용자가 좌석/주문 화면에 머무를 수 있도록 허용할 초
```

현재 Queue Server 스케줄러는 tick마다 `QUEUE_ADMIT_LIMIT_PER_TICK`명씩 active로 승격한다. `QUEUE_ACTIVE_TTL`은 active member TTL이자 admission token 만료 시간으로, 사용자가 Ticket Server 예매 API에 머무를 수 있는 시간을 의미한다.
