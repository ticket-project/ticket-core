...
진입점, `usecase`는 요청 단위 조립과 트랜잭션 경계(조회 파라미터·커서·응답 항목·DB 집계 결과도 여기 있다), `port`는
...
받는다 — 락 계약은 `booking.concurrency`다. 쓰는 곳이 하나뿐인 값은 그 곳에 둔다
(`booking.order.usecase.RequestedSeatIds`). entity 감사 컬럼만 module이 갖지 않고
