...
2. 무엇으로 막았는지(또는 막을 것인지) 적는다. **문서 한 줄보다 테스트나 훅이 낫다.**
...
**막는 방법**: 명령을 `:bootstrap:bootRun`으로 고친다. 근본 원인은 문서가 코드에서 확인 가능한
사실을 손으로 적어 둔 것이므로, `docs/architecture.md`의 모듈·패키지 나열도 함께 걷어낸다.
...
**막는 방법**: 결정표를 `/place-code` 스킬로 옮겨 원본을 한 곳으로 모은다. 개수를 세어 부르는
표현은 지운다 — 세는 순간 어긋난다.
