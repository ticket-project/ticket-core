...
## 2026-09-10 — 응답 봉투 의존을 오류 코드 값 의존으로 확대 해석했다

**무엇을**: 오류 코드 전환을 설명하면서 `ticket-fe`도 `E4001` 같은 값으로 분기한다고 말했다.
현재 소스에서 해당 분기는 확인되지 않았고, `src/lib/api.ts`는 HTTP 상태를 기준으로 처리한다.
E-code 값을 직접 비교하는 사용처는 `gatling-test`의 `SeatContentionSimulation`에서 확인했다.

**왜**: 응답 봉투를 사용하는 것과 `error.code`의 특정 값을 비교하는 것을 구분하지 않았다.

**막은 방법**: `docs/testing.md`에 외부 계약 영향 분석 시 응답 구조·HTTP 상태·오류 코드 값의
의존을 나누고 소비자 소스의 직접 비교 지점을 확인하는 규칙을 추가했다. **아직 자동화하지
못했다** — 소비자별 실제 사용처를 수동으로 확인해야 한다.
