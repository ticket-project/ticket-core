...
member  -> (없음)
admission -> (없음)
metadata  -> catalog, booking, member
...
config    -> member (공개 계약 AuthenticatedMember만. 전역 배선 module, 아래 9번 참고)
```

순환은 없다. `member`/`admission`은 다른 업무 모듈에 의존하지 않는 leaf 모듈이다. `catalog`는
찜(showlike) 흡수로 회원 존재 확인을 위해 member를 참조한다(§11). `booking`이 그 위에 얹히며,
`metadata`가 세 모듈의 공개 계약을 조합만 한다. `shared`와 `web`은 어떤 모듈도 참조하지 않는
leaf고, `config`는 member/booking의 특정 internal package와 shared를 참조하지만 반대로
...
그 목록에는 업무 모듈 의존만 남긴다. `allowedDependencies`를 비운 모듈(catalog/member/admission)은
...
`catalog.BookingPolicyLookup`, `member.MemberLookup`, `catalog.ShowLookup`)를 호출해 얻는다.
...
`BookingMetadata`, `MemberMetadata`)만 주입받고 어떤 모듈의 internal enum도 import하지 않는다.

**해결된 예외**: `showlike`는 한때 별도 module이었고 write 경로(`AddShowLikeUseCase` 등)만
`com.ticket.showlike`로, entity와 read 경로는 `member ↔ showlike`/`catalog ↔ showlike` 순환
...
등록은 값을 쓰는 member의 `SecurityConfig`가 `@EnableConfigurationProperties`로 한다),
...
**현재 상태**: 처음에는 `JpaAuditingConfig`/`SecurityContextAuditorAware`(member의
`AuthenticatedMember` 참조)가 여기 남았었지만, `com.ticket.config` module이 `@NamedInterface`로
member/booking의 필요한 internal만 좁게 열 수 있음을 확인한 뒤(§9) 그쪽으로 다시 옮겼다. 그
...
왔다)과, member의 공개 계약만 쓰는 `JpaAuditingConfig`/`SecurityContextAuditorAware`가 있다.
...
처음에는 `config`가 `WebConfig`/`WebSocketConfig`/`HttpServiceConfig`/`JwtConfig`로 member·booking의
물건을 대신 등록해 주면서 `member :: security`·`member :: oauth2`·`member :: token`·
...
그 결과 `allowedDependencies`는 `{"member"}` 하나로 줄었고 `@NamedInterface`는 하나도 남지
않았다(`seed`가 member의 `member.command`를 참조하는 것은 별개 결정이다).
...
`@EnableConfigurationProperties`로 등록한다(member의 `SecurityConfig`도 같은 타입을 등록하지만
...
- **`sharedModules`로 선언한다**: `shared`·`error`와 같은 관례다(§3). 각 module의
  ...
  `ModularityTests.APPROVED_DEPENDENCY_DAG`가 고정한다(현재 admission·booking·catalog·member·
...
소유하게 했다. §4가 "알려진 예외"로 기록했던 `member ↔ showlike`/`catalog ↔ showlike` 순환을
해소하는 결정이다.
...
목록"(`GET /api/v1/members/me/likes`)은 회원 관점 조회라 member에 남기는 방안도 검토했지만,
그러면 member가 catalog의 찜 데이터를 조회해야 해서(member → catalog) catalog의 회원 확인
참조(catalog → member)와 만나 순환이 그대로 재발한다. 그래서 찜에 관한 모든 것을 한 module에
모아 순환의 여지 자체를 없앴다.

**바뀐 것**:
- `ShowLike.member`를 member `Member`에 대한 `@ManyToOne`에서 scalar `memberId` column으로
  바꿨다(§4의 "모듈 간 참조는 scalar ID와 공개 API로만" 원칙). `ShowLike.show`는 같은 module
  안이라 `@ManyToOne` 그대로다.
- `catalog`가 회원 존재 확인을 위해 member의 `MemberLookup`을 참조한다(§3의 DAG에 반영,
  단방향). booking이 `Order.memberId`를 위해 member를 참조하는 것과 같은 패턴이다.
- `GET /api/v1/members/me/likes`의 controller가 member에서 catalog로 옮겨졌다. URL 문자열은
...
- **legacy `com.ticket.core`/`storage`/`support`**: `com.ticket.core`는 §11의 찜(showlike) 흡수로
  ...
  정식 module로 두고, 부하 테스트 회원만 member가 `@NamedInterface("seed")`로 좁혀 연
