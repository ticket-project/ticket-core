...
- `com.ticket.booking.infrastructure.redis.RedissonHoldStoreIntegrationTest`: hold 생성의 부분 실패
  보상이 좌석 키·회차별 점유 인덱스·메타데이터를 실제로 어떤 상태로 남기는지(Testcontainers)
- `com.ticket.booking.infrastructure.persistence.HoldReleaseProgressRecorderAdapterIntegrationTest`:
