...
`PerformanceSeatQueryRepository`에서 각각 정확히 한 번만 조회해 조합한다(N+1 없이 고정된
...
- 테스트 클래스 이름은 대상 클래스 이름 + `Test`로 맞춘다. Controller 계약 테스트는
  ...
  `...ScenarioTest`를 쓴다.
- 도메인 규칙과 use case는 Spring 컨텍스트 없이 검증한다. port와 구체 `*QueryRepository`는 fake나
  mock으로 대체한다(`*QueryRepository`는 interface가 아니지만 Mockito가 class도 mock한다). 다른
