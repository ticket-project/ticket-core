...
- 티켓 서버는 기존 Auth 로직과 예매 도메인을 유지하고, 대기열이 필요한 회차에서 admission token을 검증한 사용자만 좌석/홀드/주문 API에 진입시킨다.

**적용 범위**

- 포함: 대기열 서버 분리, 대기열 Redis 모델, admission token 계약, 티켓 서버 admission gate, API Gateway routing, 프론트엔드 polling 흐름
- 제외: 익명 대기열, 결제 도메인, 관리자 대기열 정책 UI, MQ/worker 기반 주문 저장 비동기화

## 1. 확정된 방향

- 대기열 진입 자체는 로그인 검증 없이 처리한다.
- 대기열 단위는 1차 구현에서 `performanceId`로 둔다.
- 대기열 참가 식별자는 클라이언트 계약에서 `queueEntryId`를 제거하고 `queueSessionId`로 단순화한다.
- 대기 순서는 Redis `ZSET`으로 관리한다.
- 입장 가능 사용자는 Redis active member 저장소로 관리한다.
- 대기열 서버는 accessToken을 검증하지 않고 Redis에 저장된 queue session을 기준으로 처리한다.
...
- 대기열 적용 여부는 Queue Server가 아니라 Ticket Server가 `PerformanceQueuePolicy` 별도 테이블의 회차 정책으로 판단한다.
- Queue Server는 자신에게 라우팅된 요청을 대기열 대상으로 보고, Queue Redis의 `queue:{performanceId}:policy` snapshot으로 waiting/active/session/admission runtime만 처리한다.
...
4. Ticket Server
   공연 상세 응답의 회차별 entryType으로 DIRECT 또는 QUEUE를 제공
...
6. Queue Server
   인증 검증 없이 Redis waiting ZSET에 performanceId + queueSessionId 등록
   queueSessionId 발급 후 Redis session에 performanceId 저장
...
12. Ticket Server
    ...
    대기열이 필요한 회차면 admissionToken 서명, 만료, performanceId 검증
...
- admission token 검증
- 회차별 대기열 적용 여부 판단
- 공연 상세 응답에 회차별 `entryType`, `queueRequired`, `queueEnterUrl`, `redirectUrl` 제공

현재 JWT 발급/검증 로직은 `core-api`의 `JwtTokenService`에 있다. Queue Server는 단순화된 현재 설계에서 JWT를 검증하지 않고, Ticket Server 예매 API가 기존 JWT 인증을 담당한다.

### 3.2 Queue Server

Queue Server는 로그인 기능과 JWT 검증을 직접 가지지 않는다.
...
Queue Server는 회원 DB나 주문 DB에 직접 접근하지 않는다. 또한 회차가 대기열 대상인지 여부를 판단하지 않는다. `performanceId`별 대기열 적용 정책은 Ticket Server가 소유하고, Queue Server는 내부 API로 받은 Queue Redis snapshot을 조회해 자신에게 라우팅된 회차의 waiting/active/session/admission runtime만 처리한다. Queue Server 설정의 `admitLimitPerTick`, `maxActiveUsers`, `activeTtl`, `sessionTtl`, `pollAfterSeconds`는 snapshot이 없는 회차에만 적용되는 fallback 정책이다.
...
│   ├── config          # app.queue와 admission token 설정
│   ├── domain          # queue model과 port
│   └── infra           # Redis/Redisson, distributed lock AOP, signed admission token 발급 구현
```

Ticket Server의 `support/security` 책임:
...
Access token 발급과 검증 책임은 Ticket Server 내부 Auth에 남긴다. Queue Server는 access token을 발급하거나 검증하지 않는다. admission token은 양 서버가 동일 issuer/audience/secret 계약을 공유하고, Queue Server는 발급만, Ticket Server는 검증만 담당한다.
...
- accessToken: Ticket Server만 검증
- refreshToken: Ticket Server만 처리
- Queue Server: refresh endpoint를 제공하지 않음
- FE: accessToken은 Ticket Server 요청의 `Authorization` 헤더에 사용
- FE: Queue Server `enter/status`에는 accessToken을 보내지 않고 `queueSessionId`를 사용
- FE: refreshToken은 기존처럼 Ticket Server refresh 흐름에서만 사용

현재 JWT는 HS256 shared secret 기반이다. 1차 구현에서는 Ticket Server 내부 인증 경계에서만 사용한다.
...
TTL 기본값은 5분으로 둔다. 실제 값은 Queue Server의 `app.queue.active-ttl` 설정으로 관리한다. 이 값은 active member TTL이자 Ticket Server 예매 API 사용 가능 TTL이다.
...
queue:member:{performanceId}:{queueSessionId}
queue:session:{queueSessionId}
queue:waiting:performances
```
...
현재 기술 스택이 Redisson이므로 1차 구현은 `RSetCache<String>` 또는 동등한 구조를 사용한다. 표준 Redis 호환성을 더 중시하면 active도 `ZSET(queueSessionId, expiresAtEpochMillis)`로 두고 scheduler가 만료 사용자를 정리한다.

### 7.3 member state

```text
key: queue:member:{performanceId}:{queueSessionId}
value: WAITING 또는 ACTIVE
ttl: queue session ttl 또는 active ttl
```
...
`queueSessionId`는 Queue Server가 생성하는 충분히 긴 랜덤 값이다. Redis lookup handle이며, JWT처럼 매 요청마다 서명 검증하는 토큰이 아니다.

Queue Server는 accessToken을 검증하지 않는다. `enter`와 `status`는 `queueSessionId`로 Redis session을 조회하고, session에 저장된 `performanceId`로 waiting/active 상태를 계산한다.

`queueSessionId`가 탈취되면 대기 상태 조회와 ACTIVE 상태의 admission token 수신 시도가 가능하다. 단순화된 현재 구조에서 admission token은 회원에 묶지 않고 회차와 만료 시간만 검증한다. 필요하면 후속 단계에서 Queue Server JWT 파싱 또는 Ticket Server-Queue Server 연계 검증으로 회원 바인딩을 되돌린다.
...
- enter는 새 `queueSessionId`를 만들고 waiting에 등록한다.
- enter 응답에는 position/예상 시간/polling 간격을 포함하지 않는다.
...
명시적 leave API는 제공하지 않는다. 클라이언트 종료는 신뢰할 수 없으므로 TTL 만료와 scheduler 처리 과정의 stale waiting 정리를 기본 정리 수단으로 둔다.
...
```text
1. performanceId별 Queue Redis policy snapshot 조회
2. active 인원을 정리한 뒤 `maxActiveUsers` 잔여분과 `admitLimitPerTick` 중 작은 수만큼 waiting 앞쪽 queueSessionId 조회
3. waiting에서 제거
4. member state가 WAITING이면 active set에 activeTtl과 함께 저장
5. member state를 ACTIVE로 갱신하고 activeTtl을 부여
```
...
6. path performanceId와 admission token performanceId 비교
7. 통과 시 controller 진입
```
...
- performance 불일치: `ADMISSION_TOKEN_INVALID`
...
1. 사용자가 예매 버튼을 누른다.
2. FE가 이미 조회한 공연 상세 응답의 선택 회차 `entryType`을 확인한다.
...
### 1단계: Ticket Server admission 기반

- `support:security`의 admission token 검증 유틸 유지
- Ticket Server 기존 로그인/refresh 흐름 유지
- Queue Server admission token 발급 계약과 Ticket Server 검증 계약 정렬
...
- `queueSessionId` 기준 enter/status로 변경
- waiting ZSET member를 `queueSessionId`로 변경
...
- 보호 대상 컨트롤러에서 `AdmissionTokenValidator` 명시 호출
- admission token 에러 타입 추가
- 기존 queue token 기반 테스트를 admission token 기반으로 변경

### 6단계: Frontend 계약 반영

- 예매 버튼에서 공연 상세 응답의 선택 회차 `entryType`으로 DIRECT/QUEUE 분기
...
- scheduler는 `maxActiveUsers` 한도 안에서 `admitLimitPerTick`만큼 waiting 앞쪽 사용자를 active로 승격한다.
