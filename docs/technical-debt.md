...
| TD-14 | 설계 | `venue.domain` | `Venue.gapX`/`gapY`(좌석 간격)를 읽는 코드가 없다 | 죽은 컬럼일 가능성 | 소비자 없음을 재확인 후 컬럼 제거 여부 결정 | 컬럼 drop은 운영 migration이라 신중히 별도 결정 | 응답 영향 없음(비노출 필드) | 컬럼 제거 또는 실제 소비자 확인 | `Venue`, `VenueSnapshot.SeatMapLayout` |
...
측정 기준: `master` (2026-09-19, 구조 리팩터링 머지 후 재측정), `src/main/java` 448파일
18,587줄. `.worktrees/` 제외.
...
남은 항목은 OE-01·OE-02·OE-03·OE-06·OE-07·OE-10 여섯이고 합계 약 −1,600줄(main의 9%),
의존성 −1이다. OE-04·OE-05·OE-08·OE-09·OE-11은 완료했다 — 무엇을 어떻게 지웠는지는 `git log`가
원본이다.
...
**보류 — 지우지 않는다**: `payment` module(8파일 292줄)은 module 밖 호출자가 없지만 죽은 코드가
아니다. `src/main/resources/db/migration-vendor/{h2,oracle}/payment/V1__create_payments.sql`이 실제 `payments` 테이블을
...
**확인했지만 후보가 아닌 것**: `package-info.java` 96개(`@NullMarked`와 modulith `ApplicationModule`
...
**측정이 말하는 것**: main의 인터페이스 65개 중 구현이 2개 이상인 것은 0개다. 10줄 미만 파일이
76개다. 위 항목을 다 적용해도 이 비율 자체는 남는다 — 다음 module 추출 때 타입을 먼저 만들지
말라는 신호로 읽는다.
