...
| TD-11 | `show.domain.performance` | `Performance` 한 entity가 회차 일정·예매 정책·대기열 정책 세 관심사를 겸한다. 예매·대기열 정책의 유일한 소비자가 booking이다 | 소유 BC(Show)와 실제 소비자(Booking)가 다르다 | A1(Show 안 `@Embeddable` 분리) 또는 A2(정책을 Booking BC로 이관) | A2는 `show -> booking` 엣지가 순환이 되어 별도 설계 필요 | 스키마·API 불변(A1) 또는 migration 필요(A2) | ADR 0006에서 방안 선택 | `Performance`, `BookingPolicySnapshot`, `BookingEntryResolver` |
