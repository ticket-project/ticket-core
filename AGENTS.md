...
Application Module이다. 실제 모듈 목록, 의존 DAG, `internal`·legacy 패키지 규칙은
`docs/architecture.md`와 `docs/adr/0003-spring-modulith-application-module-boundaries.md`가 원본이다.

**무엇이 금지인지는 문서가 아니라 구조 테스트가 원본이다.** `com.ticket.ModularityTests`가 모듈
경계 위반을 잡는다. 각 모듈 내부의 `web`/`application`/`domain`/`infrastructure` 배치는 관례가
정하고, 어떤 테스트가 무엇을 고정하는지는 `docs/testing.md`의 구조 테스트 표에 있다. 새 코드의
위치, 자주 틀리는 지점, 테스트가 실패했을 때 볼 곳은 **`/place-code` 스킬**이 원본이다.
