...
/**
 * 시드 테스트가 보는 저장소 경로다. {@code seedTest} 작업이 넘겨주는 {@code seed.project-dir}를 우선 쓰고, 없으면 현재 작업 디렉터리를 저장소 루트로 본다(IDE 실행).
 */
final class SeedTestPaths {
    ...
    static Path projectDir() {
        ...
        final Path candidate =
                configured == null || configured.isBlank() ? Path.of("").toAbsolutePath() : Path.of(configured);
    ...
    static Path minimalSeedSql() {
        final Path sqlPath =
                projectDir().resolve(Path.of("seed", "src", "test", "resources", "sql", "minimal-curated.sql"));
