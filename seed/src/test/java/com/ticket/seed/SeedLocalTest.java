...
class SeedLocalTest {
    ...
    void 빈_스키마에_공용_데이터와_부하_테스트_데이터를_적재한다() {
        assertThat(SeedLocalMain.execute()).as("정상 적재는 종료 코드 0").isZero();
    ...
    void 같은_명령을_다시_실행해도_데이터가_중복되지_않는다() {
        ...
        assertThat(SeedLocalMain.execute()).as("이미 적재된 상태에서도 정상 종료").isZero();
    ...
    private static Map<String, Long> snapshot() {
        return Map.of(
                ...
                count("PERFORMANCES"),
                "PERFORMANCE_GRADES",
                count("PERFORMANCE_GRADES"),
                "PERFORMANCE_SEATS",
                count("PERFORMANCE_SEATS"),
                "BOOKING_PERFORMANCE_SALES_POLICIES",
                count("BOOKING_PERFORMANCE_SALES_POLICIES"),
                "GRADES",
                count("GRADES"),
                "MEMBERS",
                count("MEMBERS"));
