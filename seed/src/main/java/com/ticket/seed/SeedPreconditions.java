 ...
 *
 * <p><b>스키마를 만들지 않는다.</b> 테이블 생성·삭제·초기화는 이 프로그램의 범위가 아니다. 로컬은 애플리케이션이 만들고(local 프로파일의 {@code
 * ddl-auto: create}), 운영은 Flyway migration이 이미 만들어 둔 것을 전제로 한다.
 *
 * <p><b>확인 범위는 실제로 접속한 스키마다.</b> Oracle에서 {@link DatabaseMetaData#getTables}는 접속 계정이 볼 수 있는 다른 스키마의
 * 동명 테이블까지 돌려준다 — 그것을 "준비됐다"고 읽으면 INSERT는 엉뚱한 곳을 보거나 권한 오류로 실패한다. 그래서 Oracle에서는 {@code USER_TABLES}
 * / {@code USER_TAB_COLUMNS}로 접속 계정 소유 객체만 본다.
 */
final class SeedPreconditions {
    ...
    /**
     * 회원 적재가 쓰는 {@code MEMBERS} 컬럼이다. 앱의 회원 매핑이 바뀌면 여기서 먼저 드러난다 — {@link LoadTestMemberSeeder}의
     * INSERT가 조용히 어긋나는 것보다 낫다.
     */
    ...
    private static void verifyMemberColumns(final Connection connection, final boolean oracle)
            throws SQLException {
    ...
    private static Set<String> readTableNames(final Connection connection, final boolean oracle)
            throws SQLException {
        if (oracle) {
    ...
    private static Set<String> readColumnNames(
            final Connection connection, final String table, final boolean oracle)
            throws SQLException {
        if (oracle) {
            return queryNames(
                    connection,
    ...
    private static Set<String> queryNames(final Connection connection, final String sql)
            throws SQLException {
        final Set<String> names = new LinkedHashSet<>();
