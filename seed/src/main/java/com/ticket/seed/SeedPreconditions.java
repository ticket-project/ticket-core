 ...
 *
 * <p><b>스키마를 만들지 않는다.</b> 테이블 생성·삭제·초기화는 애플리케이션의 몫이다 (local 프로파일의 {@code ddl-auto: create}).
 */
    ...
    static void verify(final DataSource dataSource, final String jdbcUrl) {
        try (Connection connection = dataSource.getConnection()) {
            ...
            throw new SeedFailure(
                    ...
                    """
                            .formatted(SeedConsole.maskedUrl(jdbcUrl), exception.getMessage()),
                    exception);
        }
    }

    private static void verifyTables(final DatabaseMetaData metaData, final String jdbcUrl)
            throws SQLException {
