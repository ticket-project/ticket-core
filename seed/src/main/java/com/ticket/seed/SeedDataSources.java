 ...
 *
 * <p>드라이버를 이름으로 직접 적재한다. {@code DriverManagerDataSource}의 이름 기반 지연 로딩과 달리, 드라이버가 classpath에 없으면 적재를
 * 시작하기 전에 바로 드러난다 — Oracle 드라이버 누락을 "테이블이 없다"는 수백 개 실패로 만나지 않기 위해서다.
 *
 * <p>Wallet(자율운영 DB)을 쓰는 URL({@code jdbc:oracle:thin:@<tns_alias>})은 {@code TNS_ADMIN} 환경변수나 {@code
 * oracle.net.tns_admin} 시스템 프로퍼티가 가리키는 디렉터리에서 설정을 읽는다. 서버가 쓰는 설정을 그대로 쓰면 된다 — 이 클래스는 그 경로를 따로 만들지
 * 않는다.
 */
final class SeedDataSources {
    ...
    static DataSource create(final SeedSettings settings) {
        ...
        try {
            final Driver driver =
                    (Driver) Class.forName(driverClassName).getDeclaredConstructor().newInstance();
            ...
            throw new SeedFailure(
                    ...
                    """
                            .formatted(driverClassName, SeedConsole.maskedUrl(settings.jdbcUrl())),
                    exception);
        }
    }
