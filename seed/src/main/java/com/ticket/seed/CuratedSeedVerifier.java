 ...
 *
 * <p>개수 비교({@link CuratedSeedInventory})만으로는 부족하다. 기대 행 수 자체를 "이미 만들어진 좌석"에서 계산하면, 좌석이 아예 없는 공연장은
 * 기대값에서도 빠져 기대값과 실제값이 사이좋게 0이 된다 — 누락이 정상 적재로 보인다. 여기서는 개수가 아니라 <b>관계</b>를 본다: 모든 공용 공연장에 물리 좌석이
 * 있는지, 모든 공용 회차에 그 공연장의 좌석이 연결됐는지.
 *
 * <p>{@link CuratedSeedLoader}의 트랜잭션 안에서 실행한다. 위반이 하나라도 있으면 예외를 던져 적재 전체를 되돌린다 — 누락된 상태가 커밋돼 "정상
 * 완료"로 보고되는 일이 없어야 한다.
 *
