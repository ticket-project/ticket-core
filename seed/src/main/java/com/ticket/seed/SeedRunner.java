 ...
 *
 * <p>작업 하나가 실패하면 그 자리에서 멈춘다. 이미 커밋된 앞 작업은 남아 있으므로, 보고에서 "적재됨 / 건너뜀 / 실패(롤백) / 실행 안 함"을 작업 단위로 구분해 출력한다.
 */
        ...
        for (final SeedTask task : tasks) {
            ...
            try {
                ...
                reports.add(new Report(
                        task.name(), outcome.skipped() ? Status.SKIPPED : Status.LOADED, outcome.summary(), null));
            } catch (final RuntimeException exception) {
                reports.add(new Report(task.name(), Status.FAILED, "실패했습니다. 이 작업의 트랜잭션은 롤백됐습니다.", exception));
                stopped = true;
            }
        }
