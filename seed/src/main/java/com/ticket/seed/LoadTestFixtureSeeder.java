 ...
 *
 * <p>운영 Oracle의 {@code scripts/core-capacity/create-core-capacity-data.sql}과 같은 고정 ID 대역을 쓴다. 그래야
 * Gatling Console이 고정해서 넘기는 좌석 시작 ID와 회차 배정표를 대상만 바꿔 그대로 쓸 수 있다.
 *
 * <p>공용 시드({@link CuratedSeedLoader})가 먼저 실행돼 GRADES에 VIP/R/S/A code를 만들어 두면 이 시더는 그 code를 재사용한다 —
 * 같은 code로 GRADES row를 중복 생성하지 않는다.
 *
 * <p>트랜잭션은 {@link #run()} 전체를 감싼다. 중간에 실패하면 전부 되돌려야 다음 실행에서 {@code alreadySeeded()}가 반쯤 적재된 데이터를 보고
 * 건너뛰는 일이 없다.
 */
final class LoadTestFixtureSeeder implements SeedTask {
    ...
    /**
     * 회차·좌석 등급 배정에 쓰는 공통 정의다. code, name, price, sortOrder 순서다. ADR 0005 이후 등급 가격은
     * 회차(PerformanceGrade) 단위로만 존재하므로 show 단위 가격표(과거 ShowGrade)는 만들지 않는다.
     */
    ...
    public Outcome run() {
        if (performanceCount <= 0) {
            return Outcome.skipped(
                    "performance-count=%d 이므로 적재하지 않습니다.".formatted(performanceCount));
        }
        if (alreadySeeded()) {
            return Outcome.skipped(
        ...
        return Outcome.done(
                "showId=%d, 회차 %d개, 물리 좌석 %d석, 회차좌석 %d행을 적재했습니다."
                        .formatted(
                                SHOW_ID,
                                performanceCount,
                                SEAT_COUNT,
                                performanceCount * SEAT_COUNT));
    }
     ...
     * BOOKING_PERFORMANCE_SALES_POLICIES가 소유한다. FORCE_OFF는 이 부하 테스트 전용 회차가 Queue 없이 Core를 직접 호출한다는
     * 기존 의미를 그대로 보존한다.
     */
    ...
    /**
     * GRADES는 재사용 가능한 등급 코드다(ADR 0005). 공용 시드가 먼저 돌아 VIP/R/S/A code를 이미 만들어 뒀을 수 있어, code당 하나만 있도록
     * 존재하면 재사용하고 없으면 새로 만든다.
     */
    ...
    /**
     * PerformanceSeat.unitPrice의 원본은 PerformanceGrade.price다(ADR 0005) — 회차마다 PERFORMANCE_GRADES를
     * 만든다.
     */
    private void seedPerformanceGrades(
            final LocalDateTime now, final Map<String, Long> gradeIdsByCode) {
        final Timestamp createdAt = Timestamp.valueOf(now);
    ...
    private void seedPerformanceSeats(final LocalDateTime now) {
        ...
        for (int performance = 1; performance <= performanceCount; performance++) {
            final long performanceId = ID_BASE + performance;
            final List<Long> performanceGradeIds =
                    jdbcTemplate.queryForList(
