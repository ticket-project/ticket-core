...
| Ticket entity·상태, OrderSeat/Member 참조는 scalar `orderSeatId`/`ownerMemberId` | `booking.internal.domain.ticket` |
...
`booking.internal.domain.ticket`/`booking.internal.infrastructure.ticket`에 있고 역시 entity-only다. 그래서 실제로 쓰이는 계층은 `internal.domain`
